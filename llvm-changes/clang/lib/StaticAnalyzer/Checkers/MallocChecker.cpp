...
#define CHECK_FN(NAME)                                                         \
  ...
  /// \param [in] ReturnsNullOnFailure (CALTECHCS) Whether the memory
  /// deallocation function we're modeling returns with Null on failure.
  /// \param [in] State The \c ProgramState right before allocation.
  /// \returns The ProgramState right after allocation.
  [[nodiscard]] ProgramStateRef
  MallocMemAux(CheckerContext &C, const CallEvent &Call, const Expr *SizeEx,
               SVal Init, bool ReturnsNullOnFailure, ProgramStateRef State,
               AllocationFamily Family) const;
...
MallocChecker::performKernelMalloc(const CallEvent &Call, CheckerContext &C,
  ...
  if (TrueState && !FalseState) {
    ...
    return MallocMemAux(C, Call, Call.getArgExpr(0), ZeroVal, true, TrueState,
...
void MallocChecker::checkBasicAlloc(ProgramStateRef State,
                                    const CallEvent &Call,
                                    CheckerContext &C) const {
  State = MallocMemAux(C, Call, Call.getArgExpr(0), UndefinedVal(), true, State,
                       AllocationFamily(AF_Malloc));
...
void MallocChecker::checkKernelMalloc(ProgramStateRef State,
  ...
  if (MaybeState)
    ...
    State = MallocMemAux(C, Call, Call.getArgExpr(0), UndefinedVal(), true,
                         State, AllocationFamily(AF_Malloc));
  C.addTransition(State);
}
...
void MallocChecker::checkAlloca(ProgramStateRef State, const CallEvent &Call,
                                CheckerContext &C) const {
  // CALTECHCS-NOTE: Alloca has undefined behavior in the event of failure
  // (stack overflow), so there's no need to add a null transition.
  State = MallocMemAux(C, Call, Call.getArgExpr(0), UndefinedVal(), false,
                       State, AllocationFamily(AF_Alloca));
  State = ProcessZeroAllocCheck(C, Call, 0, State);
  C.addTransition(State);
}
...
void MallocChecker::checkStrdup(ProgramStateRef State, const CallEvent &Call,
  ...
  if (!CE)
    return;
  State = MallocMemAux(C, Call, UnknownVal(), UnknownVal(), true, State,
                       AllocationFamily(AF_Malloc));
...
void MallocChecker::checkCXXNewOrCXXDelete(ProgramStateRef State,
  ...
  case OO_New:
    // CALTECHCS-NOTE: ReturnsNullOnFailure should be true with std::nothrow
    // However, this will only be used for C code, so it's not important.
    State = MallocMemAux(C, Call, CE->getArg(0), UndefinedVal(), false, State,
...
void MallocChecker::checkGMalloc0(ProgramStateRef State, const CallEvent &Call,
  ...
  State = MallocMemAux(C, Call, Call.getArgExpr(0), zeroVal, true, State,
                       AllocationFamily(AF_Malloc));
  State = ProcessZeroAllocCheck(C, Call, 0, State);
  C.addTransition(State);
}
...
void MallocChecker::checkGMemdup(ProgramStateRef State, const CallEvent &Call,
                                 CheckerContext &C) const {
  State = MallocMemAux(C, Call, Call.getArgExpr(1), UnknownVal(), true, State,
                       AllocationFamily(AF_Malloc));
  State = ProcessZeroAllocCheck(C, Call, 1, State);
  C.addTransition(State);
}
...
void MallocChecker::checkGMallocN(ProgramStateRef State, const CallEvent &Call,
  ...
  State = MallocMemAux(C, Call, TotalSize, Init, true, State,
                       AllocationFamily(AF_Malloc));
  State = ProcessZeroAllocCheck(C, Call, 0, State);
  State = ProcessZeroAllocCheck(C, Call, 1, State);
  C.addTransition(State);
...
MallocChecker::MallocMemReturnsAttr(CheckerContext &C, const CallEvent &Call,
  ...
  if (!Att->args().empty()) {
    return MallocMemAux(C, Call,
                        ...
                        UndefinedVal(), false, State, Family);
  }
  return MallocMemAux(C, Call, UnknownVal(), UndefinedVal(), false, State,
                      Family);
}
...
ProgramStateRef MallocChecker::MallocMemAux(CheckerContext &C,
  ...
  assert(SizeEx);
  return MallocMemAux(C, Call, C.getSVal(SizeEx), Init, ReturnsNullOnFailure,
                      State, Family);
}
...
ProgramStateRef MallocChecker::MallocMemAux(CheckerContext &C,
  ...
  // CALTECHCS-NOTE: Check whether -analyzer-config cfg-malloc-failure=true flag
  // was specified
  const AnalysisDeclContext *ADC = C.getCurrentAnalysisDeclContext();
  if (ADC->getCFGBuildOptions().AddMallocFailure) {
    // CALTECHCS-NOTE: Check whether allocation can fail. The isa<...>(RetVal)
    // condition is there to make sure we can call RetVal.castAs<...>() on the
    // next line.
    if (ReturnsNullOnFailure && isa<DefinedOrUnknownSVal>(RetVal)) {
      DefinedOrUnknownSVal location = RetVal.castAs<DefinedOrUnknownSVal>();
      // CALTECHCS-NOTE:
      // State->assume will return two new states with assumptions about the
      // condition (which is that the return value of the allocation is not
      // null): one where the condition is assumed to be true and one where it
      // is assumed to be false (in that order). Each of those states will be
      // falsy if the analyzer can conclude that the state is unreachable (in
      // other words, that the condition necessary to reach that state cannot
      // be true) and truthy otherwise.
      ProgramStateRef nullState, notNullState;
      std::tie(notNullState, nullState) = State->assume(location);
      // CALTECHCS-NOTE: If the return value of allocation might be null, add
      // the corresponding transition.
      if (nullState)
        C.addTransition(nullState);
      // CALTECHCS-NOTE: If the return value of allocation must be null (the
      // notNullState is unreachable), there's nothing else to consider, so
      // return early.
      if (!notNullState)
        return nullptr;
      // CALTECHCS-NOTE: The transition to the notNullState will be added by the
      // caller when the state is returned, so we only need to update State, not
      // call addTransition.
      State = notNullState;
    }
  }
...
MallocChecker::ReallocMemAux(CheckerContext &C, const CallEvent &Call,
  ...
  if (PrtIsNull && !SizeIsZero) {
    ProgramStateRef stateMalloc = MallocMemAux(
        C, Call, TotalSize, UndefinedVal(), true, StatePtrIsNull, Family);
    return stateMalloc;
  }
  ...
  if (ProgramStateRef stateFree =
    ...
    ProgramStateRef stateRealloc = MallocMemAux(
      C, Call, TotalSize, UnknownVal(), false, stateFree, Family);
    if (!stateRealloc)
      return nullptr;
...
ProgramStateRef MallocChecker::CallocMem(CheckerContext &C,
  ...
  return MallocMemAux(C, Call, TotalSize, zeroVal, true, State,
                      AllocationFamily(AF_Malloc));
}
